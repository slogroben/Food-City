<template>
    ...
    <!-- <table>
        ...
        <tr v-for="s in sellerList" :key="s.id">
            ...
            <td>{{s.sellerphone}}</td>
            <td>
                <button @click="quitpass(s)">允许退出</button>
                <button @click="quitreject(s)">拒绝退出</button>
            </td>
        </tr>
    </table> -->
  </div>
...
export default {
    ...
    methods:{
        quitpass(s){
            axios({
                method:'get',
                url:'http://localhost:8080/My/AdminIsQuitSellerServlet?shop_id='+s.shop_id+'&shopstate='+'quitpass'
                })
                .then(
                    response=>{
        ...
        quitreject(s){
            axios({
                method:'get',
                url:'http://localhost:8080/My/AdminIsQuitSellerServlet?shop_id='+s.shop_id+'&shopstate='+'quitreject'
                })
                .then(
                    response=>{
                        this.sellerList=response.data
        ...
        getUser(){
                axios({
                method:'get',
                url:'http://localhost:8080/My/AdminFindQuitSellerServlet?shopstate='+'isquit'
                })
                .then(
                    response=>{
                        this.sellerList=response.data
